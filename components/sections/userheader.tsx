...
export function Header() {
  ...
  const DropdownMenu = ({ items, isOpen }: { items: typeof navItems[0]['items'], isOpen: boolean }) => (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="absolute left-0 mt-2 w-48 rounded-md bg-black/95 py-2 backdrop-blur-md"
        >
          {items.map((item) => (
            <Link
              key={item.name}
            ...
            <nav className="flex flex-col space-y-4 py-4">
              {navItems.map((item) => (
                <div key={item.name} className="space-y-2">
                  <button
                    onClick={() => handleDropdownToggle(item.name)}
                    className="flex w-full items-center justify-between text-neutral-300 transition-colors hover:text-yellow-500"
                  >
                    <span>{item.name}</span>
                    <ChevronDown
                      className={`h-4 w-4 transition-transform ${
                      ...
                      >
                        {item.items.map((subItem) => (
                          <Link
                            key={subItem.name}
                            href={subItem.href}
                            className="block text-sm text-neutral-300 transition-colors hover:text-yellow-500"
                            onClick={() => {
